...
你只管跟 Claude 提需求。它把任务拆成独立的工单，派给你电脑上现成的便宜 AI 命令行工具（Cursor、Gemini、通过 pi 调 DeepSeek/GLM、Codex 等）在后台写代码。做完之后，Claude 自己重新跑一遍验收命令，再调另一个厂商的模型来挑错，挑出来的bug Claude 还要逐条去翻代码核实，确认没问题才收工。

Claude 的额度全花在**刀刃**上：架构思考、拆单、跑验收、核对审查意见。出力的脏活累活交给便宜的按量付费模型。
...
- **异族代码审查**：作者在自己一个项目中：13 张deepseek工单，glm 3 轮审查，挑出 36 个问题；Claude 逐条核实后，采纳并修好 35 个，驳回 1 个，成本极低，性价比极高。
